...
function renderWaConvList(){
  const prosp=AUTORIZADOR.puede('ver_global')?DS.find('prospectos'):DS.find('prospectos',{asesor:CU.id});
...
function sendWaMsg(pid){
  ...
  seguimientosService.crear({prospectoId:pid,tipo,nota:txt,fecha:new Date().toISOString(),usuario:CU.id,estatusCambio:null});
...
function addWaTag(pid,tag){
  ...
  // Future: prospectosService.actualizar(pid,{tags:[...(p.tags||[]),tag]})
}
...
function simulateNewMsg(){
  ...
  seguimientosService.crear({prospectoId:p.id,tipo:'WhatsApp',nota:msg,fecha:new Date().toISOString(),usuario:p.asesor,estatusCambio:null});
  renderWaConvList();
...
function openWaConfig(){
  const cfg=(configService.obtenerExt('va_wa_config')||{});
...
function saveWaConfig(){
  ...
  configService.guardarExt('va_wa_config',cfg);
  closeM('m-wa-config');
