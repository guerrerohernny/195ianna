...
function confirmarSeparacion(){
  ...
  if($('sep-modo-reactivar')?.checked){
    ...
    inventarioService.actualizarPorClave(orig,{estado:'Disponible',
...
function saveLote(){
  ...
  if(clave){
    const loteAct=getLote(clave);
    if(loteAct){
      const hist=[...(loteAct.historial||[])];
      if(loteAct.estado!==estFinal) hist.push({...histEntry});
      inventarioService.actualizarPorClave(clave,{mz,lote:loteNum,terreno:parseFloat(ter.toFixed(3)),excedente:parseFloat(exc.toFixed(3)),plusvalia:plus,tipo_ubicacion:tipo,tipo,estado:estFinal,valor_terreno,modelo_asignado:modeloAsg,cliente_asignado:clienteAsg,fecha_operacion:fechaOp,dir_oficial:$('le-dir-oficial').value.trim(),historial:hist});
    }
...
function deleteLote(){
  ...
  inventarioService.eliminarPorClave(clave);
  closeM('m-lote-edit'); renderInventario(); populateSelects();
