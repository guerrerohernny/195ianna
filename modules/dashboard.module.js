...
function renderDashboard(){
  ...
  const allP=AUTORIZADOR.puede('ver_global')?DS.find('prospectos'):DS.find('prospectos',{asesor:CU.id});
  ...
  const isG=AUTORIZADOR.puede('ver_global');
  const kpis=isG?[
  ...
  const allPN=AUTORIZADOR.puede('ver_global')?DS.find('prospectos'):DS.find('prospectos',{asesor:CU.id});
  ...
  // Citas de hoy
  const citasHoy=DS.find('recordatorios').filter(r=>(AUTORIZADOR.puede('ver_global')||r.usuario===CU.id)&&r.fecha===todayStr&&r.tipo.includes('Cita')&&r.estado==='pendiente');
  ...
  const waPend=DS.find('recordatorios').filter(r=>(AUTORIZADOR.puede('ver_global')||r.usuario===CU.id)&&r.estado==='pendiente'&&(r.tipo.includes('WhatsApp')||r.tipo.includes('Seguimiento')));
  ...
  const recVenc=DS.find('recordatorios').filter(r=>(AUTORIZADOR.puede('ver_global')||r.usuario===CU.id)&&r.estado==='pendiente'&&new Date(r.fecha+'T'+(r.hora||'23:59'))<new Date());
...
function calcMisIngresos(){
  if(!CU) return '$0';
  // 1.95 (Etapa 4): la comisión SOLO sale del Motor de Comisiones — misma cifra que Ingresos y que el ledger.
  const ventas=apartadosService.ventas().filter(a=>a.asesor===CU.id);
  const total=ventas.reduce((s,a)=>s+IANNA_COM.comisionAsesor(a).total,0);
  return mxn(total);
}
