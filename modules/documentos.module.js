...
function imprimirPagares(){
  if(!_cierreData) return;
  const {l,m,plazo} = _cierreData;
  // ── 1.95 (Etapa 5 · CAUSA RAÍZ): cada pagaré imprime SU folio congelado.
  //    Al cerrar la venta, apartados congela pagares_congelados con folio único
  //    e id REC- propio por pagaré. La impresión leía el folio de carátula y lo
  //    repetía en todos — eso queda eliminado. Antes del cierre es VISTA PREVIA
  //    y no consume ni muestra folios (los folios solo se emiten al congelar). ──
  const apAct = apartadosService.obtener(_cierreData.ap?.id) || _cierreData.ap || {};
  const congelados = apAct.pagares_congelados;
  const esPreview = !(congelados && congelados.length);
  const pagares = esPreview ? (_cierreData.pagares||[]) : congelados;
  if(!pagares.length) return;
  const cli = getClienteData();
  const win = window.open('','_blank');
  const _fmtFechaPag = (f)=>{ const d=(f instanceof Date)?f:new Date(String(f).slice(0,10)+'T12:00:00'); return isNaN(d)?'—':d.toLocaleDateString('es-MX',{day:'2-digit',month:'long',year:'numeric'}); };
  const pagaresHTML = pagares.map((p,idx)=>`
    <div style="border:2px solid #1E3D0F;border-radius:8px;padding:16px;margin-bottom:20px;page-break-inside:avoid;${idx<pagares.length-1?'page-break-after:always;':''}">
      <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:12px">
        ...
        <div style="text-align:center;font-weight:800;font-size:14px;color:#1E3D0F">PAGARÉ ${p.n}/${p.total||plazo}</div>
  ...
  <h2 style="text-align:center;color:#1E3D0F;margin-bottom:20px">PAGARÉS — ${cli.nombre}${esPreview?' <span style=\"font-size:12px;color:#b45309\">(VISTA PREVIA — los folios se emiten al cerrar la venta)</span>':''}</h2>
  ${pagaresHTML}
