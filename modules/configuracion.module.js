...
function saveAsesor(){
  ...
  if(aid){ const patch={nombre:nm,correo:em,telefono:fmtTelVal($('as-ph').value),activo:$('as-act').value==='true'}; if(pw){ if(pw.length<4){ toast('Contraseña mín. 4 caracteres','err'); return; } patch.pass=pw; } usuariosService.actualizar(aid,patch); toast('Asesor actualizado ✓','ok'); }
  else { if(!pw||pw.length<4){ toast('Contraseña requerida (mín. 4)','err'); return; } if(DS.find('usuarios').some(u=>u.correo===em)){ toast('Ese correo ya existe','err'); return; } usuariosService.crear({nombre:nm,correo:em,pass:pw,telefono:fmtTelVal($('as-ph').value),rol:'asesor',activo:true,fechaAlta:new Date().toISOString(),avatar:''}); toast('Asesor creado ✓','ok'); }
...
function deleteAsesor(){
  ...
  if(IANNA_MOTOR.asesorEnUso(aid)){
    ...
    usuariosService.actualizar(aid,{activo:false});
...
async function connectSupabase(){
  ...
  try {
    ...
    if(res.ok||res.status===400){
      configService.guardarExt('va_sb_url',url); configService.guardarExt('va_sb_key',key);
