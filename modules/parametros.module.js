...
let _distDraft={asesor:[],gerente:[]}; let _cobroDistDraft={credito:null,contado:null,especiales:[]}; let _esquemasComisionDraft=[]; let _captacionDraft=[]; let _distTemporalDraft=[]; let _esquemasComercialesDraft=[]; let _esquemasPagoDraft=[];
function renderPoliticaComercial(){
  ...
  const d=pol.distribuciones_cobro||IANNA_COM.politicaDefault().distribuciones_cobro; _cobroDistDraft=JSON.parse(JSON.stringify(d)); renderCobroDistribuciones(); _captacionDraft=JSON.parse(JSON.stringify(pol.esquemas_captacion||IANNA_COM.politicaDefault().esquemas_captacion||[])); _distTemporalDraft=JSON.parse(JSON.stringify(pol.distribuciones_temporales||IANNA_COM.politicaDefault().distribuciones_temporales||[])); _esquemasComisionDraft=JSON.parse(JSON.stringify(pol.esquemas_comision||[])); _esquemasComercialesDraft=JSON.parse(JSON.stringify((IANNA_COM.esquemasComercialesDisponibles?IANNA_COM.esquemasComercialesDisponibles(pol):(pol.esquemas_comerciales||[])))); _esquemasPagoDraft=JSON.parse(JSON.stringify(pol.esquemas_pago||((typeof IANNA_MIG_1974!=='undefined')?IANNA_MIG_1974.defaults():[]))); renderEsquemasComision();
...
// 1.97.4 — Esquema padre por modalidad con fuentes y distribución independiente por rol (1 a 4 partes).
function _pctPartesOk(arr){return arr.length>=1&&arr.length<=4&&Math.abs(arr.reduce((a,x)=>a+Number(x.pct||0),0)-1)<0.0001}
function _renderPartesRol(ei,fi,rol){
  const f=_esquemasPagoDraft[ei].fuentes[fi]; f.distribuciones=f.distribuciones||{}; f.distribuciones[rol]=f.distribuciones[rol]||[{parte:'firma',nombre:'Firma',evento:'contrato_firmado',pct:1}];
  const arr=f.distribuciones[rol];
  return `<div style="border:1px solid var(--bd);border-radius:8px;padding:8px;background:#fff"><div style="display:flex;justify-content:space-between;align-items:center"><b style="font-size:11px">Pago ${rol==='asesor'?'Asesor':rol==='gerente'?'Gerente':'Tercero'}</b><span style="font-size:10px;color:${_pctPartesOk(arr)?'#047857':'#b91c1c'}">${_pctVal(arr.reduce((a,x)=>a+Number(x.pct||0),0))}</span></div>${arr.map((p,pi)=>`<div style="display:grid;grid-template-columns:1.15fr .55fr 28px;gap:4px;margin-top:5px"><input value="${p.nombre||''}" oninput="_esquemasPagoDraft[${ei}].fuentes[${fi}].distribuciones.${rol}[${pi}].nombre=this.value"><input type="number" step="0.001" value="${_pctVal(p.pct)}" oninput="_esquemasPagoDraft[${ei}].fuentes[${fi}].distribuciones.${rol}[${pi}].pct=(parseFloat(this.value)||0)/100"><button class="btn btn-red btn-xs" ${arr.length<=1?'disabled':''} onclick="_esquemasPagoDraft[${ei}].fuentes[${fi}].distribuciones.${rol}.splice(${pi},1);renderEsquemasComision()">×</button></div>`).join('')}<button class="btn btn-out btn-xs" style="margin-top:6px" ${arr.length>=4?'disabled':''} onclick="_esquemasPagoDraft[${ei}].fuentes[${fi}].distribuciones.${rol}.push({parte:'parte_'+Date.now(),nombre:'Nueva parte',evento:'manual',pct:0});renderEsquemasComision()">+ Parte</button></div>`;
}
function renderEsquemasComision(){
  const el=$('pc-esquemas-comision'); if(!el)return;
  const rows=(_esquemasPagoDraft||[]).map((e,ei)=>`<div class="card" style="padding:14px;margin:12px 0;border-top:5px solid #173a6a"><div style="display:grid;grid-template-columns:1.5fr .8fr auto;gap:8px;align-items:end"><div class="fg" style="margin:0"><label>Nombre del esquema</label><input value="${e.nombre||''}" oninput="_esquemasPagoDraft[${ei}].nombre=this.value"></div><div class="fg" style="margin:0"><label>Modalidad</label><select onchange="_esquemasPagoDraft[${ei}].modalidad=this.value"><option value="credito" ${e.modalidad==='credito'?'selected':''}>Crédito</option><option value="contado" ${e.modalidad==='contado'?'selected':''}>Contado</option><option value="especial" ${e.modalidad==='especial'?'selected':''}>Especial</option></select></div><button class="btn btn-red btn-xs" onclick="_esquemasPagoDraft.splice(${ei},1);renderEsquemasComision()">Eliminar</button></div><div style="margin-top:12px;overflow:auto"><table class="tbl" style="min-width:1150px"><thead><tr><th>Fuente</th><th>Asesor %</th><th>Gerente %</th><th>Tercero</th><th>% tercero</th><th>Distribución por rol</th><th></th></tr></thead><tbody>${(e.fuentes||[]).map((f,fi)=>`<tr><td><input value="${f.nombre||''}" oninput="_esquemasPagoDraft[${ei}].fuentes[${fi}].nombre=this.value" style="min-width:160px"><select onchange="_esquemasPagoDraft[${ei}].fuentes[${fi}].canal=this.value" style="margin-top:4px"><option value="directo" ${f.canal==='directo'?'selected':''}>Personal</option><option value="broker" ${f.canal==='broker'?'selected':''}>Bróker</option><option value="digital" ${f.canal==='digital'?'selected':''}>Digital</option><option value="guardia" ${f.canal==='guardia'?'selected':''}>Guardia</option><option value="corporativo" ${f.canal==='corporativo'?'selected':''}>Corporativo</option><option value="recomendacion" ${f.canal==='recomendacion'?'selected':''}>Recomendación</option></select></td><td><input type="number" step="0.001" value="${_pctVal(f.porcentajes?.asesor)}" oninput="_esquemasPagoDraft[${ei}].fuentes[${fi}].porcentajes.asesor=(parseFloat(this.value)||0)/100" style="width:78px"></td><td><input type="number" step="0.001" value="${_pctVal(f.porcentajes?.gerente)}" oninput="_esquemasPagoDraft[${ei}].fuentes[${fi}].porcentajes.gerente=(parseFloat(this.value)||0)/100" style="width:78px"></td><td><input value="${f.tercero_tipo||''}" oninput="_esquemasPagoDraft[${ei}].fuentes[${fi}].tercero_tipo=this.value" style="width:110px"></td><td><input type="number" step="0.001" value="${_pctVal(f.porcentajes?.tercero||f.porcentajes?.broker)}" oninput="_esquemasPagoDraft[${ei}].fuentes[${fi}].porcentajes.tercero=(parseFloat(this.value)||0)/100;_esquemasPagoDraft[${ei}].fuentes[${fi}].porcentajes.broker=(parseFloat(this.value)||0)/100" style="width:78px"></td><td><div style="display:grid;grid-template-columns:repeat(3,minmax(230px,1fr));gap:6px">${_renderPartesRol(ei,fi,'asesor')}${_renderPartesRol(ei,fi,'gerente')}${_renderPartesRol(ei,fi,'tercero')}</div></td><td><button class="btn btn-red btn-xs" onclick="_esquemasPagoDraft[${ei}].fuentes.splice(${fi},1);renderEsquemasComision()">×</button></td></tr>`).join('')}</tbody></table></div><button class="btn btn-out btn-xs" style="margin-top:8px" onclick="addFuenteEsquema(${ei})">+ Agregar fuente</button></div>`).join('');
  el.innerHTML=`<div style="font-weight:800;font-size:14px">Esquemas de pago de comisiones</div><div style="font-size:11.5px;color:var(--t3);margin:5px 0 10px">Cada esquema contiene sus fuentes, porcentajes por rol y una distribución independiente de 1 a 4 partes para Asesor, Gerente y Tercero.</div>${rows||'<div class="empty"><p>Sin esquemas.</p></div>'}<button class="btn btn-gold btn-xs" onclick="addEsquemaComercial()">+ Nuevo esquema</button>`;
}
function addFuenteEsquema(ei){_esquemasPagoDraft[ei].fuentes.push({id:'fuente_'+Date.now(),nombre:'Nueva fuente',canal:'directo',porcentajes:{asesor:0.02,gerente:0.005,tercero:0,broker:0},tercero_tipo:'',distribuciones:{asesor:[{parte:'firma',nombre:'Firma',evento:'contrato_firmado',pct:1}],gerente:[{parte:'firma',nombre:'Firma',evento:'contrato_firmado',pct:1}],tercero:[{parte:'escritura',nombre:'Escritura',evento:'escriturado',pct:1}]}});renderEsquemasComision()}
function addEsquemaComercial(){const base=(_esquemasPagoDraft[0]||((typeof IANNA_MIG_1974!=='undefined')?IANNA_MIG_1974.defaults()[0]:null));_esquemasPagoDraft.push(JSON.parse(JSON.stringify({...base,id:'ESQ-'+Date.now(),nombre:'Nuevo esquema',modalidad:'especial'})));renderEsquemasComision()}
function addEsquemaCaptacion(){addEsquemaComercial()} function addDistribucionTemporal(){addEsquemaComercial()} function addEsquemaComisionEspecial(){addEsquemaComercial()}
function validarEsquemasComision(){if(!_esquemasPagoDraft.length){toast('Agrega al menos un esquema','err');return false}for(const e of _esquemasPagoDraft){if(!(e.fuentes||[]).length){toast(`El esquema ${e.nombre} no tiene fuentes`,'err');return false}for(const f of e.fuentes){for(const rol of ['asesor','gerente','tercero']){const arr=f.distribuciones?.[rol]||[];const pct=Number(f.porcentajes?.[rol]||0);if(pct>0&&!_pctPartesOk(arr)){toast(`${e.nombre} / ${f.nombre}: la distribución de ${rol} debe tener 1 a 4 partes y sumar 100%`,'err');return false}}}}return true}
