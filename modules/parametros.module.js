...
function renderPoliticaComercial(){
  ...
  if($('pc-pct-ad')) $('pc-pct-ad').value = (pol.porcentajes.asesor_directo*100).toLocaleString('es-MX',{maximumFractionDigits:3});
  if($('pc-pct-ab')) $('pc-pct-ab').value = (pol.porcentajes.asesor_broker*100).toLocaleString('es-MX',{maximumFractionDigits:3});
  if($('pc-pct-ge')) $('pc-pct-ge').value = (pol.porcentajes.gerente*100).toLocaleString('es-MX',{maximumFractionDigits:3});
  if($('pc-pct-bk')) $('pc-pct-bk').value = (pol.porcentajes.broker*100).toLocaleString('es-MX',{maximumFractionDigits:3});
...
function validarEsquemasComision(){if(!_esquemasPagoDraft.length){toast('Agrega al menos un esquema','err');return false}for(let ei=0;ei<_esquemasPagoDraft.length;ei++){const e=_esquemasPagoDraft[ei];if(!(e.fuentes||[]).length){toast(`El esquema ${e.nombre} no tiene fuentes`,'err');return false}for(let fi=0;fi<e.fuentes.length;fi++){const f=e.fuentes[fi];for(const rol of ['asesor','gerente','tercero']){const arr=_normalizarPartesFuente(ei,fi,rol),total=Number(f.porcentajes?.[rol]||0),sum=arr.reduce((a,x)=>a+Number(x.pct_venta||0),0);if(arr.length<1||arr.length>4||Math.abs(sum-total)>.00001){toast(`${e.nombre} / ${f.nombre}: las partes de ${rol} deben sumar ${_pctVal(total)}`,'err');return false}}}}return true}
