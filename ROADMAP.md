...
## Fase 1.95 — Kernel Stabilization Release ✅ COMPLETADA (2026-07-07)

Estabilización total del núcleo antes de la evolución SaaS. Sin funcionalidades
nuevas: una sola vía para cada responsabilidad.

- **Datos:** capa de Services única puerta de escritura (0 `DS.create/update/delete`,
  0 mutaciones `DS.db`, 0 `localStorage` fuera del DataStore). `schema_version:'1.95'`
  sembrado en cada registro (preparación Fase 2).
- **Estados:** `IANNA_PIPELINE` — flujo único (kanban ≡ ficha ≡ edición ≡ importación);
  'Apartado'/'Venta' solo nacen de la Operación (derivación auditada); la venta no
  retrocede por arrastre.
- **Autorización:** motor `AUTORIZADOR` (AUT-v1, fail-closed, trazable) — 0 `rol===`.
- **Dinero:** el ledger es la única fuente de saldo (reconciliación histórica
  idempotente); comisiones solo de `IANNA_COM`.
- **Documentos:** folio congelado por pagaré (+`emitirUnaVez` idempotente), vistas
  previas sin folio, reimpresión segura.
- **Parámetros:** reorganizado en 6 categorías (preparado para multiempresa).
- **Verificación:** suite de motores (41 casos) + 9 guardas de CI (`scripts/guards.sh`)
  + guion E2E (`tests/E2E_CHECKLIST.md`). ADR-015…019 en `docs/DECISIONS.md`.

> Nota de registro: la "suite de 67 pruebas" citada en documentación previa no
> venía incluida en el paquete 1.9; la suite oficial reproducible del proyecto es
> la de esta fase.
