...
/* ── Fase 1.95 (Etapa 2): separación Persona ↔ Operación ──
   ESTATUS_CRM: asignables manualmente (pipeline comercial de la Persona).
   ESTATUS_OPERACION: DERIVADOS de una Operación real — jamás se escriben a mano;
   los produce el Motor de Operaciones (crear apartado / contrato firmado). */
const ESTATUS_CRM=['Nuevo','Contactado','Cita agendada','Visitó desarrollo','Seguimiento'];
const ESTATUS_OPERACION=['Apartado','Venta'];
const ESTATUS_ACTIVOS=ESTATUS_CRM.concat(ESTATUS_OPERACION); // columnas visibles del kanban
...
function populateSelects(){
  ...
  const misB=AUTORIZADOR.puede('ver_global')?DS.find('brokers',{activo:true}):DS.find('brokers').filter(b=>b.asesorId===CU.id&&b.activo);
  ...
  const prs=AUTORIZADOR.puede('ver_global')?DS.find('prospectos'):DS.find('prospectos',{asesor:CU.id});
...
function buildAlerts(){
  ...
  DS.find('recordatorios').filter(r=>r.estado==='pendiente'&&(AUTORIZADOR.puede('ver_global')||r.usuario===CU.id)).forEach(r=>{
