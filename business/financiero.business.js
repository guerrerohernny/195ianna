...
window.IANNA_FIN = (function(){
  ...
  function registrarMovimiento(m){
    ...
    const mov = {
      ...
      motivo:            m.motivo || '',
      fecha_valor:       m.fecha_valor || null, // fecha original del hecho (reconciliaciones); la fecha del asiento nunca se altera
    };
  ...
  /* ──────────────────────────────────────────────────────────────
     RECONCILIACIÓN HISTÓRICA (Fase 1.95 · Etapa 4 — ADR-011)
     Operaciones previas al ledger (Fase <1.9) tienen pagos solo en
     ap.pagos[]/monto_enganche. Para que el ledger sea la ÚNICA
     fuente de saldo, se les crean asientos de ingreso equivalentes.
     · Idempotente por documento: si ya existe un MOV con el mismo
       (operacionId, documento), no se duplica.
     · Nunca modifica asientos existentes (append-only intacto).
     ────────────────────────────────────────────────────────────── */
  function reconciliarHistorico(){
    const res={creados:0, operaciones:0};
    try{
      const aps=DS.find('apartados');
      aps.forEach(ap=>{
        const movs=movimientosDe(ap.id);
        const tiene=(doc)=>movs.some(m=>m.documento===doc);
        let creo=false;
        // Enganche/apartado inicial
        if((ap.monto_enganche||0)>0){
          const docEng='ENG-'+(ap.id_publico||ap.id);
          if(!tiene(docEng)){
            registrarMovimiento({tipo:'ingreso', operacionId:ap.id, personaId:ap.prospectoId,
              monto:ap.monto_enganche, metodo:ap.metodo_pago||'Transferencia', documento:docEng,
              concepto:'Apartado inicial (reconciliación histórica)',
              politica_version:(ap.politica_snapshot||{}).version||null,
              fecha_valor:ap.fecha_apartado||null,
              motivo:'Reconciliación 1.95: asiento equivalente del enganche registrado antes del ledger'});
            res.creados++; creo=true;
          }
        }
        // Pagos de cobranza
        (ap.pagos||[]).forEach(p=>{
          const doc=IANNA_FMT.FOLIO(p.folio);
          if(!tiene(doc) && (p.monto||0)>0){
            registrarMovimiento({tipo:'ingreso', operacionId:ap.id, personaId:ap.prospectoId,
              monto:p.monto, metodo:p.metodo||'Transferencia', documento:doc,
              concepto:(p.concepto||'Pago')+' (reconciliación histórica)',
              politica_version:(ap.politica_snapshot||{}).version||null,
              fecha_valor:p.fecha||null,
              motivo:'Reconciliación 1.95: asiento equivalente de pago registrado antes del ledger'});
            res.creados++; creo=true;
          }
        });
        if(creo) res.operaciones++;
      });
      if(res.creados) try{ IANNA_MOTOR.auditar('ledger','reconciliacion_v195','RECONCILIACION_HISTORICA',{},res,'Fase 1.95: el ledger pasa a ser la única fuente de saldo'); }catch(e){}
    }catch(e){ console.error('reconciliarHistorico',e); }
    return res;
  }
