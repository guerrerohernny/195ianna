...
window.IANNA_FOLIOS = {
  ...
  // 1.95 (Bloque 5): emisión IDEMPOTENTE por referencia — si ya se emitió un folio
  // para (tipo, referencia), devuelve el mismo. Es la garantía dura de que congelar
  // pagarés (u otros documentos únicos) jamás pueda duplicar folios, ni siquiera si
  // el flujo que congela se ejecutara dos veces por error.
  emitirUnaVez(tipo, referencia){
    const ref=String(referencia||'');
    const previo=(DS.db.folios_registro||[]).find(r=>r.tipo===(tipo||'documento') && r.ref===ref);
    if(previo) return String(previo.folio).padStart(8,'0');
    return this.emitir(tipo, referencia);
  },
