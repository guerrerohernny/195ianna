...
# Apéndice — Fase 1.95 (Kernel Stabilization)

**Flujo canónico único** (Bloque 1, implementado):
`UI (solicita) → AUTORIZADOR → Máquina de Estados / IANNA_PIPELINE → Motor de Operaciones → Motores (FIN/COM/FOLIOS/FMT) → Services → DataStore → IANNA_SYNC → Auditoría`

**Nuevos componentes:** `business/autorizador.business.js` (política AUT-v1),
`business/pipeline.business.js` (estatus CRM + derivación operacional),
`modules/operaciones-ui.module.js` (presentación del Motor de Operaciones).

**Invariantes vigilados por CI** (`scripts/guards.sh`, G1–G9): módulos sin
escrituras DS ni mutación de `DS.db`; `localStorage` solo en el DataStore; cero
comparaciones de rol; comisiones solo de `IANNA_COM`; saldo solo del ledger;
formato solo de `IANNA_FMT`; estados operacionales solo derivados; motor sin UI.

**Orden de carga corregido:** `entidades.service.js` carga inmediatamente después
de `dataStore.service.js` (antes cargaba después de `app.init.js`).
